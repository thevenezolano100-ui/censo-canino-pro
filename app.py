...
app = Flask(__name__)
app.secret_key = 'clave_secreta_censo_master_v15'

# ==========================================
# RUTAS ABSOLUTAS Y BASE DE DATOS (v15)
# ==========================================
BASE_DIR = os.path.abspath(os.path.dirname(__file__))
app.config['UPLOAD_FOLDER'] = os.path.join(BASE_DIR, 'static', 'uploads')
os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)

db_path = os.path.join(BASE_DIR, 'censo_pro_v15.db')
...
class Usuario(UserMixin, db.Model):
    ...
    username = db.Column(db.String(50), unique=True, nullable=False); password = db.Column(db.String(200), nullable=False)
    rol = db.Column(db.String(20), default='Ciudadano'); nombre = db.Column(db.String(100)); apellido = db.Column(db.String(100))
    cedula = db.Column(db.String(20)); whatsapp = db.Column(db.String(20)); direccion = db.Column(db.Text)

@login_manager.user_loader
def load_user(user_id): return Usuario.query.get(int(user_id))

class Carrusel(db.Model): id = db.Column(db.Integer, primary_key=True); imagen = db.Column(db.String(200), nullable=False)
class Video(db.Model): id = db.Column(db.Integer, primary_key=True); archivo = db.Column(db.String(200), nullable=False)
class ManualDoc(db.Model): id = db.Column(db.Integer, primary_key=True); archivo = db.Column(db.String(200), nullable=False)
class Medicamento(db.Model): id = db.Column(db.Integer, primary_key=True); nombre = db.Column(db.String(100), nullable=False); stock = db.Column(db.Integer, default=0); unidad = db.Column(db.String(50), nullable=False)
...
class NotaHospitalizacion(db.Model):
    ...
    temperatura = db.Column(db.String(20)); frecuencia_card = db.Column(db.String(20)); evolucion = db.Column(db.Text, nullable=False)
    veterinario = db.Column(db.String(100)); canino_id = db.Column(db.Integer, db.ForeignKey('canino.id'), nullable=False)

class Cita(db.Model):
    id = db.Column(db.Integer, primary_key=True); fecha_hora = db.Column(db.DateTime, nullable=False); motivo = db.Column(db.String(200), nullable=False)
    estado = db.Column(db.String(20), default='Pendiente'); canino_id = db.Column(db.Integer, db.ForeignKey('canino.id'), nullable=False)

class Vacuna(db.Model):
    id = db.Column(db.Integer, primary_key=True); tipo = db.Column(db.String(100), nullable=False); fecha_aplicacion = db.Column(db.Date, nullable=False)
    fecha_proxima = db.Column(db.Date, nullable=False); canino_id = db.Column(db.Integer, db.ForeignKey('canino.id'), nullable=False)

class RegistroReproductivo(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    tipo_evento = db.Column(db.String(100), nullable=False); fecha_evento = db.Column(db.Date, nullable=False)
...
class Canino(db.Model):
    ...
    latitud = db.Column(db.Float); longitud = db.Column(db.Float); situacion = db.Column(db.String(100), default='Censo Normal')
    reportado_por = db.Column(db.String(100)); cubiculo_jaula = db.Column(db.String(50), default="N/A") 
    
    esterilizado = db.Column(db.Boolean, default=False); desparasitado = db.Column(db.Boolean, default=False); vacuna_parvovirus = db.Column(db.Boolean, default=False)
    vacuna_moquillo = db.Column(db.Boolean, default=False); vacuna_triple = db.Column(db.Boolean, default=False)
    vacuna_sextuple = db.Column(db.Boolean, default=False); vacuna_antirrabica = db.Column(db.Boolean, default=False)
...
def sincronizar_offline():
    ...
    for item in datos:
        try: lat = float(item.get('latitud') or 0); lon = float(item.get('longitud') or 0)
        except: lat = 0; lon = 0
        nuevo = Canino(
            especie=item.get('especie', 'Canino'), fase_reproductiva='Ninguna',
...
def inicio():
    if request.method == 'POST':
        ...
        try: lat = float(request.form.get('latitud') or 0); lon = float(request.form.get('longitud') or 0)
        except: lat = 0; lon = 0
        situacion_recibida = request.form.get('situacion', 'Censo Normal')
        if situacion_recibida == 'Hospitalizado' and current_user.rol != 'Admin': situacion_recibida = 'Censo Normal'

        nuevo = Canino(
            especie=request.form.get('especie', 'Canino'), fase_reproductiva='Ninguna', foto=n_f, nombre=request.form.get('nombre'), raza=request.form.get('raza'), edad=request.form.get('edad'), 
...
def editar(id):
    ...
    if request.method == 'POST':
        ...
        p.especie = request.form.get('especie', 'Canino')
...
def eliminar(id):
    if current_user.rol != 'Admin': return redirect(url_for('inicio'))
    db.session.delete(Canino.query.get_or_404(id)); db.session.commit(); flash('Paciente eliminado.', 'success'); return redirect(url_for('inicio'))
...
# ============================================================
# MODULO DE CITAS, ALERTAS Y HOSPITALIZACION
# ============================================================
...
def eliminar_nota_hospitalizacion(id):
    if current_user.rol != 'Admin': return redirect(url_for('inicio'))
    db.session.delete(NotaHospitalizacion.query.get_or_404(id)); db.session.commit(); flash('Nota eliminada.', 'success'); return redirect(url_for('hospitalizacion_pizarra'))

@app.route('/hospitalizacion/alta/<int:id>')
@login_required
def dar_alta_hospitalizacion(id): 
    if current_user.rol != 'Admin': return redirect(url_for('inicio'))
    p = Canino.query.get_or_404(id); p.situacion = 'Censo Normal'; p.estado_salud = 'Sano'; db.session.commit(); flash('Alta médica ejecutada.', 'success'); return redirect(url_for('hospitalizacion_pizarra'))
...
def exportar():
    ...
    writer.writerow(['ID', 'Especie', 'Nombre', 'Raza', 'Sector', 'Salud', 'Propietario', 'WhatsApp'])
    for p in Canino.query.all(): writer.writerow([p.id, p.especie, p.nombre, p.raza, p.sector, p.estado_salud, p.nombre_propietario, p.whatsapp_propietario])
